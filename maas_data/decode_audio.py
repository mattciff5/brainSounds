...
def decode(args):
    ...
    pipe = StableAudioPipeline.from_pretrained(
        args.stable_audio_id, torch_dtype=torch.float32
    ).to(device)
    ...
    model.duration_embedding.load_state_dict(ckpt["duration_embedding"])
    model.eval()
    ...
    for batch in tqdm(loader, desc="Decoding"):
        ...
        # 1) Ottieni i conditioning tokens
        prompt_embeds = model.encode_brain(brain)      # [B, 128, 768]

        # Encode durata con la pipeline originale (start=0, end=target_duration_s)
        seconds_start_hs, seconds_end_hs = pipe.encode_duration(
            audio_start_in_s = 0.0,
            audio_end_in_s   = args.target_duration_s,
            device           = device,
        ...
        # Concatena: text_audio_duration_embeds = [B, 130, 768]
        text_audio_duration_embeds = torch.cat(
            [prompt_embeds, seconds_start_hs, seconds_end_hs], dim=1
        )

        # audio_duration_embeds = [B, 1, 1536]
        ...
        # 5) Decode → waveform
        waveform_start = 0
        waveform_end   = int(args.target_duration_s * pipe.vae.config.sampling_rate)

        audio_out = vae.decode(latents).sample                        # [B, 2, samples_full]
        ...
        for i in range(B):
            ...
            torchaudio.save(out_path, wav, args.target_sr)
            ...
            if args.save_groundtruth:
                ...
                torchaudio.save(gt_path, gt_wav, args.target_sr)
