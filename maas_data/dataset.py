...
class AudioNeuroDataset(Dataset):
    ...
    def __init__(
        ...
        self.normalize_brain = normalize_brain
    ...
    def _normalize_brain(self, x: torch.Tensor) -> torch.Tensor:
        """Z-score per-sample su tutti i voxel."""
        mean = x.mean()
        std  = x.std()
        if std > 1e-8:
            x = (x - mean) / std
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        if self.normalize_brain:
            brain = self._normalize_brain(brain)
...
def build_datasets(
    ...
    train_ds = AudioNeuroDataset(
        train_roi, train_idx, all_wav_paths, target_sr, target_len_s
    )
    test_ds = AudioNeuroDataset(
        test_roi, test_idx, all_wav_paths, target_sr, target_len_s
    )
