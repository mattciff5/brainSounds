...
def train(args):
    ...
    for epoch in range(args.num_epochs):
        ...
        for step, batch in enumerate(pbar):
            ...
            # b) Campiona timestep e aggiungi rumore
            B = latents.shape[0]
            ...
            # c) Predici il rumore
            noise_pred = model(brain, noisy_lat, timesteps)          # [B, C, T]

            # d) Loss (predizione di rumore standard, opz. v-prediction)
            if noise_scheduler.config.prediction_type == "epsilon":
                target = noise
            elif noise_scheduler.config.prediction_type == "v_prediction":
                target = noise_scheduler.get_velocity(latents, noise, timesteps)
            else:
                raise ValueError(f"Prediction type non supportato: "
