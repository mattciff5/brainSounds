...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, query_embed, pos_embed, masks=False, src_all=None):
        # flatten NxCxHxW to HWxNxC
        bs, c, h, w = src.shape
        src = src.flatten(2).permute(2, 0, 1) # [num_parcels, B, d_model]
        pos_embed = pos_embed.flatten(2).permute(2, 0, 1) # [num_parcels, B, d_model]
        ...
        if self.num_encoder_layers > 0:
            memory_layers, sattn = self.encoder(
...
class TransformerEncoder(nn.Module):
    ...
    def forward(
        ...
        return output.unsqueeze(0), sattn.unsqueeze(
            0
        )  # encoder tokens and self-attention weights from the last layer
...
class TransformerDecoder(nn.Module):
    ...
    def forward(
        ...
        for layer in self.layers:
            # assert not torch.isnan(output).any(), "NaNs in fmri_tokens!"
            output = layer(
                output,
                memory,
                tgt_mask=tgt_mask,
            ...
            if self.return_intermediate:
                intermediate.append(self.norm(output))
            # assert not torch.isnan(memory).any(), "NaNs in decoder_queries!"
...
class TransformerDecoderLayer(nn.Module):
    ...
    def with_pos_embed(self, tensor, pos: Optional[Tensor]):
        # print("with_pos_embed", tensor.shape, pos.shape if pos is not None else None)
    ...
    def forward_post(
        ...
        tgt2 = self.multihead_attn(
            ...
            key=self.with_pos_embed(memory, pos).detach(),
            value=memory,
